...
class SpotDataFetcher(BaseFetcher):
    ...
    def get_candles(
        ...
        if verbose:
            quote_str = f" ({quote.upper()})" if quote else ""
            logger.info(f"[现货 K 线] {exchange.upper()} {base.upper()}{quote_str} | {len(markets)} 个市场 | {frequency}")
        ...
        if verbose:
            logger.info(f"[现货 K 线] 完成：{len(df)} 条")
        return df
