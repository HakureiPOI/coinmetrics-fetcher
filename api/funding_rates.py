...
提供 Deribit 等交易所永续合约资金费率和预计资金费率的便捷获取功能。
"""
...
class FundingRateFetcher:
    ...
    def get_all_funding_rates(
        ...
        Returns:
            包含资金费率和预计资金费率的 DataFrame
        """
        if verbose:
            logger.info("=" * 60)
            logger.info(
                f"获取 {exchange.upper()} {base.upper()} 永续合约资金费率 + 预计资金费率"
            )
            logger.info("=" * 60)
            logger.info(f"时间范围：{start_time} 至 {end_time}")
        ...
        if len(funding_df) > 0 and len(predicted_df) > 0:
            # 合并资金费率和预计资金费率
            merged_df = pd.merge(
                funding_df,
                predicted_df,
        ...
        elif len(funding_df) > 0:
            result = funding_df
        elif len(predicted_df) > 0:
            result = predicted_df
        else:
            result = pd.DataFrame()
